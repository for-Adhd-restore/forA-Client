...
const MedSelectModal: React.FC<MedSelectModalProps> = ({
    onClose,
    onSelectMed,
    savedSelectedMed,
}) => {
    ...
    const handleMedSelect = (item: MedListItem) => {
        setSelectedMed((prevSelectedMed) => {
            ...
            const isAlreadySelected = prevSelectedMed.some(
                (med) => med.medicineId === item.medicineId,
            );

            if (isAlreadySelected) {
                console.log('이미 선택된 약 제거:', item);
                return prevSelectedMed.filter(
                    (med) => med.medicineId !== item.medicineId,
                );
            } else {
            ...
            <ScrollView>
                {filteredMedList.map((med: MedListItem) => (
                    <ModalMedListItem
                        ...
                        isSelected={selectedMed.some(
                            (selected) =>
                                selected.medicineId === med.medicineId,
                        )}
                    />
                ))}
            </ScrollView>
